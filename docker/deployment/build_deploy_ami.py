...
def run_cmd(exec_str, description):
    ...
    if exit_code is not 0:
        print(
            '{} returned non-zero exit status. Returned status `{}`'.format(description, exit_code))
        if stdout != '':
            print('STDOUT:')
            print(stdout)
        if stderr != '':
...
parser.add_argument('--dockerhub_repo', default=None,
                    help='Release from a non-NREL DockerHub repository')
parser.add_argument('--write_json', help='Write AMI JSON specification to file instead of S3',
                    action='store_true')
parser.add_argument('--disable_public', help='Do not make the AMI public', action='store_true')
parser.add_argument('--enable_custom_build', help='Flag to allow non-standard AMI release',
                    action='store_true')
...
if override_docker_version is not None:
    if verbose:
        print('Docker version being overwritten from `{}` to `{}`'.format(docker_version,
                                                                          override_docker_version))
    docker_version = override_docker_version

# Get the OpenStudioServer version and version extension to use
version_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '../../server/app/lib/openstudio_server/version.rb')
)

# parse the version from the file directly, no need to have ruby
with open(version_path, 'r') as version_file:
    file_data = version_file.read()

    ami_version = re.search(r'VERSION\s=\s.(.*)..freeze', file_data).group(1)
    if verbose:
        print("OSS version retrieved is %s" % ami_version)

    if override_ami_version is not None:
        if verbose:
            print('AMI version being overwritten from `{}` to `{}`'.format(
                ami_version, override_ami_version)
            )
        ami_version = override_ami_version

    ami_version_ext = re.search(r'VERSION_EXT\s=\s.(.*)..freeze', file_data).group(1)

    if verbose:
        print('OSS version extension retrieved is %s' % ami_version_ext)
...
if override_dockerhub_repo is not None:
    ...
    for line in fileinput.input(docker_config_path, inplace=1, backup='.bak'):
        line = re.sub('docker pull nrel', 'docker pull {}'.format(override_dockerhub_repo),
                      line.rstrip())
...
if ami_id_line.split(':')[0] != 'us-east-1':
    raise RuntimeError(
        'Unexpected return from the packer script. Please review {}'.format(packer_log))
...
if not disable_public:
    ...
    if response['ResponseMetadata']['HTTPStatusCode'] is not 200:
        raise RuntimeError(
            'API request setting AMI {} permissions to public failed to return status code 200, instead '
...
if response['ResponseMetadata']['HTTPStatusCode'] is not 200:
    raise RuntimeError(
        'API request uploading the updated amis.json file failed to return status code 200, instead '
